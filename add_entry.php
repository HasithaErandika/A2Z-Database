...
function getEmployees($conn) {
    $employees = [];
    $sql = "SELECT Emp_ID, Emp_Name FROM Employee WHERE Date_of_resigned IS NULL";
...
function getNextAutoIncrementValue($conn, $table, $autoIncrementField) {
    $sql = "SELECT MAX($autoIncrementField) as max_id FROM $table";
    $result = $conn->query($sql);
    if ($result && $result->num_rows > 0) {
...
{
    $jobData = [];
    $sql = "SELECT Job_ID, Service_Category, Customer_ref, Location, Client_ref, Job_capacity FROM Jobs";
    $result = $conn->query($sql);
        ...
        <?php
                    ...
                    if ($result->num_rows > 0) {
                        ...
                        if ($autoIncrementField) {
                            $nextId = getNextAutoIncrementValue($conn, $table, $autoIncrementField);
                            echo '<div class="form-group">';
                            echo '<label for="' . $autoIncrementField . '">' . ucfirst(str_replace('_', ' ', $autoIncrementField)) . ' (Next ID)</label>';
                            echo '<input type="text" class="form-control" id="' . $autoIncrementField . '" name="' . $autoIncrementField . '" value="' . $nextId . '" readonly>';
                            echo '</div>';
                        }
                        ...
                        while ($row = $result->fetch_assoc()) {
                            ...
                            if ($table == 'Attendance' && $row['Field'] == 'Emp_ID') {
                                echo '<select class="form-control employee-select" name="' . $row['Field'] . '" required>';
                            ...
                            }elseif ($row['Field'] == 'Presence') {
                                echo '<div>';
                                echo '<input type="hidden" id="' . $row['Field'] . '" name="' . $row['Field'] . '" value="">';
                                echo '<button type="button" class="btn btn-full-day btn-round mr-2" onclick="setPresenceValue(\'' . $row['Field'] . '\', 1.0)">Full Day</button>';
                                echo '<button type="button" class="btn btn-half-day btn-round mr-2" onclick="setPresenceValue(\'' . $row['Field'] . '\', 0.5)">Half Day</button>';
                                echo '<button type="button" class="btn btn-not-attended btn-round" onclick="setPresenceValue(\'' . $row['Field'] . '\', 0.0)">Not Attended</button>';
                                echo '</div>';
